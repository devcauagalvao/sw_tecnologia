  ...
  return (
    <div className="p-10 bg-gradient-to-br from-gray-50 to-gray-100 min-h-screen flex flex-col items-center">
      ...
      <input
        ...
        className="mb-6 w-full max-w-md px-4 py-3 rounded-xl shadow-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"
      />
      ...
      <div className="overflow-x-auto w-full max-w-7xl">
        <table className="min-w-full border-separate border-spacing-y-2">
          <thead>
            <tr>
              <th className="px-6 py-4 text-left text-gray-600 uppercase tracking-wider text-sm">
                ID
              </th>
              <th className="px-6 py-4 text-left text-gray-600 uppercase tracking-wider text-sm">
                Nome
          ...
          <tbody>
            {filteredVlans.map((vlan, idx) => (
              <tr
                key={vlan.id}
                className={`transition-transform transform hover:scale-105 hover:shadow-lg cursor-pointer rounded-xl ${
                  idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'
                }`}
              >
                <td className="px-6 py-4 text-gray-800 font-medium">{vlan.id}</td>
                <td className="px-6 py-4 text-gray-900 font-semibold">{vlan.name}</td>
                <td className="px-6 py-4 text-gray-700">{vlan.ip}</td>
              </tr>
            ))}
